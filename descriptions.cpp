                           ...
                           std::string *name)
{
   ...
   * we'g consume newlines and perhaps miss a restart on the next      *
   * line.                                                             */
...
uint32_t parse_descriptions(_game *g, char* monster_path, char* object_path)
{
  std::string file;
  std::ifstream f;
  ...
  g->monster_descriptions = m;

  if (parse_monster_descriptions(f, g, m)) {
    delete m;
  ...
  g->object_descriptions = o;

  if (parse_object_descriptions(f, g, o)) {
    delete o;
...
uint32_t print_descriptions(_game *g)
{
  std::vector<monster_description> *m;
  ...
  m = (std::vector<monster_description> *) g->monster_descriptions;

  for (mi = m->begin(); mi != m->end(); mi++) {
    mi->print(std::cout);
  }

  o = (std::vector<object_description> *) g->object_descriptions;
...
uint32_t destroy_descriptions(_game *g)
{
...
character_t *generate_monster(_game *g, uint32_t rank)
{
  character_t *c;
  const std::vector<monster_description> &v =
    *((std::vector<monster_description> *) g->monster_descriptions);
  const monster_description &m = v[rand()%v.size()];
  uint32_t room;
  pair_t p;
  ...
  c->health = m.hitpoints.roll();
  c->damage = (const dice_t *) &m.damage;
  c->next_turn = g->pc->next_turn;
  c->alive = 1;
  c->rank = rank;
  c->pc = NULL;
  ...
  g->character[c->position.y][c->position.x]=c;
